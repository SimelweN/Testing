...
export class EdgeFunctionDebugger {
  ...
  async diagnoseFunction(functionName: string): Promise<EdgeFunctionDiagnostic> {
  ...
  private async testWithDirectFetch(functionName: string, startTime: number): Promise<EdgeFunctionDiagnostic> {
      ...
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        return {
          ...
          error: {
            ...
            details: 'The edge function URL is not responding. It may not be deployed.',
  ...
  private extractErrorDetails(error: any): string {
    ...
    if (error.status) return `HTTP ${error.status}: ${error.statusText || 'Unknown'}`;
  ...
  private printDiagnosticReport(results: EdgeFunctionDiagnostic[]) {
    ...
    results.forEach(result => {
      ...
      console.log(`${statusIcon} ${result.functionName} (${result.timing.toFixed(0)}ms)`);
      console.log(`   URL: ${result.url}`);
      
      if (result.status === 'success') {
        console.log(`   ✅ Response:`, result.response);
      } else {
        console.log(`   ❌ Status: ${result.httpStatus || 'unknown'} ${result.httpStatusText || ''}`);
        console.log(`   ❌ Error:`, result.error?.message || 'Unknown error');
        console.log(`   ❌ Details:`, result.error?.details || 'No details available');
        ...
        if (result.headers) {
          const relevantHeaders = Object.entries(result.headers)
            .filter(([key]) => ['content-type', 'x-edge-function-error', 'x-sb-gateway-error'].includes(key.toLowerCase()));
...
export async function debugEdgeFunctions(): Promise<EdgeFunctionDiagnostic[]> {
  ...
  return await debugger.diagnoseAllFunctions();
}
