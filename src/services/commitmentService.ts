...
export interface SaleCommitment {
  ...
  delivery_fee: number;
  total_amount: number;
  status: 'pending' | 'committed' | 'declined' | 'expired' | 'completed' | 'refunded';
  committed_at?: string;
  expires_at: string;
...
export const getPendingCommitments = async (sellerId: string): Promise<CommitmentWithDetails[]> => {
  try {
    ...
    if (error) {
      console.error('Error fetching pending commitments:', error.message || error);
    ...
    const userIds = [...commitments.map(c => c.seller_id), ...commitments.map(c => c.buyer_id)];
...
export const getAllCommitments = async (userId: string): Promise<CommitmentWithDetails[]> => {
  try {
    // First get the commitments
    const { data: commitments, error } = await supabase
      .from('sale_commitments')
    ...
    return commitments.map(commitment => {
      ...
      return {
        ...
        seller_name: seller?.name || 'Unknown Seller',
        buyer_name: buyer?.name || 'Unknown Buyer',
        time_remaining: commitment.status === 'pending' ? calculateTimeRemaining(commitment.expires_at) : undefined,
      };
    });
  } catch (error) {
...
export const getCommitmentStats = async (sellerId: string) => {
  try {
    ...
    const committedSales = data.filter(c => c.status === 'committed' && c.committed_at);
    ...
    if (committedSales.length > 0) {
      const totalResponseTime = committedSales.reduce((sum, sale) => {
        ...
        const diffHours = (committed.getTime() - created.getTime()) / (1000 * 60 * 60);
